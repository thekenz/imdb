view: cast_title_facts {
  ...
  derived_table: {
    ...
    sql: SELECT
      ...
      FROM `lookerdata.imdb.title` AS title
      LEFT JOIN  `lookerdata.imdb.cast_info` AS cast_info ON title.id = cast_info.movie_id
  ...
  dimension: bling {
    sql: CASE WHEN ${lifetime_movies_with_revenue} > 5 THEN 'Movie$|' ELSE '' END
